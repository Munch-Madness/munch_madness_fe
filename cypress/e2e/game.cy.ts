...
cy.get('p').eq(16).should('have.text', "McDonald's");
    ...
    cy.get('p').eq(1).should('have.text', 'Buca di Beppo Italian Restaurant');
    cy.get('p').eq(8).should('have.text', "Famous Dave's Bar-B-Que");
    cy.get('button').eq(8).click();
    cy.get('h2').should('have.text', "Famous Dave's Bar-B-Que");
    cy.get('.absolute.top-1.right-1').should('have.text', 'exit').click();
    cy.get('svg').eq(0).click();
    cy.get('svg').eq(4).click();
    cy.get('svg').eq(8).click();
    cy.get('svg').eq(12).click();
    cy.get('p').eq(1).should('have.text', 'Buca di Beppo Italian Restaurant');
    cy.get('p').eq(4).should('have.text', `Café Zupas`);
    cy.get('button').eq(4).click();
    cy.get('h2').should('have.text', `Café Zupas`);
    cy.get('.absolute.top-1.right-1').should('have.text', 'exit').click();
    cy.get('svg').eq(0).click();
    cy.get('svg').eq(4).click();
    cy.get('p').eq(1).should('have.text', 'Buca di Beppo Italian Restaurant');
    cy.get('p').eq(2).should('have.text', 'Burger King');
    cy.get('button').eq(2).click();
    cy.get('h2').should('have.text', 'Burger King');
    cy.get('.absolute.top-1.right-1').should('have.text', 'exit').click();
    cy.get('svg').eq(0).click();
...
describe('testing restaurant choice menu', () => {
  ...
  it('should allow a user to select a restaurant', () => {
    ...
    cy.get('li').last().should('have.text', `Yummy Grill & Sushi ⚪️`);
    cy.get('li').eq(0).contains('⚪️').click();
