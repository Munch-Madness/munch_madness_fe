...
function App() {
  ...
  useEffect(() => {
    fetch(`https://munch-madness-be-8b56c3719f5f.herokuapp.com/api/v1/places/?query=${zipCode}&search=random`)
  .then(response => {
    if (!response.ok) {
  ...
  return (
    <div className="App flex flex-col justify-center items-center bg-background">
      ...
      {!bracketSize && <BracketChoice setZipCode={setZipCode} setBracketSize={setBracketSize}/>}
